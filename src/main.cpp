...
bool loadMedia()
{
  ...
  if (!gButtonSpriteSheetTexture.loadFromFile("assets/images/buttons.png", gRenderer))
  {
    ...
    else
    {
      ...
      while(!quit)
      {
        ...
        for (int i = 0; i < TOTAL_BUTTONS; ++i)
        {
          ...
          gButtons[i].render(gRenderer, gButtonSpriteSheetTexture, currentButtonDisplay);
        }
